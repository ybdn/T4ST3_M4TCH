...
const ExternalSearchBar: React.FC<ExternalSearchBarProps> = ({
  ...
  const [selectedResult, setSelectedResult] =
    useState<ExternalSearchResult | null>(null);
  ...
  const handleSearch = useCallback(async (searchQuery: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/search/external/?q=${encodeURIComponent(
          searchQuery
        )}&limit=8`,
        {
          headers: {
  ...
  return (
    <Combobox value={selectedResult} onChange={handleSelectResult}>
      <div className="relative w-full">
        ...
        >
          <Combobox.Options
            className="absolute z-10 mt-1 w-full tm-glass-card rounded-xl shadow-xl overflow-auto scrollbar-hide"
              ...
              results.map((result) => (
                ...
                >
                  ...
                  <div className="flex-grow min-w-0">
                    <h4 className="font-semibold text-tm-text truncate">
                      {result.title}
                    </h4>
