...
const ExternalSearchBar: React.FC<ExternalSearchBarProps> = ({ 
  ...
  const handleSearch = async (searchQuery: string) => {
    ...
    setIsLoading(true);
    try {
      // Simulation de recherche - remplacez par votre logique d'API
              ...
              results.map((result) => (
                ...
                >
                  ...
                  <div className="flex-grow min-w-0">
                    ...
                    <p className="text-sm text-tm-text-muted line-clamp-2">{result.description}</p>
                    <div className="flex items-center gap-2 mt-1 text-xs">
                      <span className="px-2 py-0.5 bg-tm-primary/20 text-tm-primary rounded-full">
                        {result.category_display}
                      </span>
                      <span className="px-2 py-0.5 bg-white/10 text-tm-text-muted rounded-full">
                        {result.source.toUpperCase()}
                      </span>
                      {result.release_date && (
                        <span className="text-tm-text-muted">
                  ...
                  {onQuickAdd && (
                    <button 
                      ...
                      className="ml-4 p-2 tm-glass-button rounded-full text-tm-primary hover:text-white transition-colors duration-200"
                    >
