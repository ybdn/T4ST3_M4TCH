...
const ExplorePage: React.FC<ExplorePageProps> = ({ onNavigate }) => {
  ...
  // const [searchResultsCount, setSearchResultsCount] = useState<number>(0); // (désactivé: non utilisé pour l'instant)
  ...
  const fetchTrending = async (category?: string) => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/trending/external/?${params.toString()}`,
        {
          headers: {
  ...
  const handleAddFromTrending = async (
    item: TrendingItem | ExternalSearchResult
  ) => {
    const token = localStorage.getItem("access_token");
        ...
        <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6 relative">
          ...
          <section
            className={`transition-all duration-300 ${
              searchSuggestionsVisible
                ? "fixed top-16 left-0 right-0 z-20 px-4 sm:px-6 lg:px-8"
                : ""
            }`}
          >
            <div
              className={searchSuggestionsVisible ? "max-w-7xl mx-auto" : ""}
            >
              <ExternalSearchBar
                onSelect={(r: ExternalSearchResult) => handleAddFromTrending(r)}
                onQuickAdd={(r: ExternalSearchResult) =>
                  handleAddFromTrending(r)
                }
          ...
          <section
            className={`tm-glass-card rounded-xl p-6 transition-all duration-300 ${
              searchSuggestionsVisible
              ...
              <TabPanels>
                {categories.map((category) => (
                  <TabPanel key={category.key}>
                    {error && (
                      <div
                        className="tm-glass border border-red-400/40 text-red-400 p-4 rounded-xl text-sm text-center mb-6"
                        style={{
                          boxShadow:
                      ...
                      >
                        {loading
                          ? Array.from({ length: 8 }).map((_, idx) => (
                              <div
                                key={idx}
                                className="tm-glass rounded-xl overflow-hidden flex-shrink-0 w-64 h-96"
                              >
                          ...
                          : trending
                              ...
                              .map((item, index) => (
                                <div
                                  ...
                                  style={{
                                    boxShadow:
                                      "inset 0 1px 0 rgba(255, 255, 255, 0.15), 0 4px 12px rgba(255, 255, 255, 0.05)",
                                  }}
                                >
                    ...
                    {!loading && trending.length === 0 && !error && (
                      <div
                        className="tm-glass rounded-xl p-8 text-center"
                        style={{
                          boxShadow:
                            "inset 0 1px 0 rgba(255, 255, 255, 0.15), 0 4px 12px rgba(255, 255, 255, 0.05)",
                        }}
          ...
          >
            ...
            >
              ...
              <p className="text-tm-text-muted mb-4 leading-relaxed">
                Créez des listes et likez du contenu pour recevoir des
